...
class DB {
	...
	static public function _($key = 0) {
		if (!isset($mysqli[$key])) {
			...
			self::$mysqli[$key] = @new mysqli(self::$connect['server'], self::$connect['user'], 
				self::$connect['pass'], self::$connect['db']);

			if (mysqli_connect_errno()) {
				echo "Failed to connnect to the DataBase";
			}
			
			if (self::$mysqli[$key]->set_charset("utf-8")) {
				echo "Error while loading symbols utf-8: ".self::$mysqli[$key]->error;
				exit();
			}
